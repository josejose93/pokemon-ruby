...
class Pokemon
  attr_reader :type, :effort_points, :moves, :current_move, :growth_rate
  attr_accessor :experience_points, :stats, :stat_effort
  # (complete parameters)
  def initialize(name = "Colita", species, level = 1)
    # Retrieve pokemon info from Pokedex and set instance variables
    pokemon_details = Podekex::POKEMONS[species]
    
    @name = name
    @species = species
    @type = pokemon_details[:type]
    @effort_points = pokemon_details[:effort_points]
    @base_stat = pokemon_details[:stats]
  ...
  # IMPORTANTE: Retorna un hash con propiedades del movimiento. Revisar si esa es la consigna o no
  ...
  def attack(target)
    ...
    if hits
      ...
      # -- Effectiveness check
      effectiveness = TYPE_MULTIPLIER.find { |hash| hash[:user] == @current_move[:type] && hash[:target] == target.type }
  ...
  def get_stats(**base_stat)
    ...
    base_stat.keys.each do |key|
      stats[key] =
      ((2 * base_stat[key] + @stat_individual_values[key] + @stat_effort[key]) * @level / 100 + 5).floor
    end
    stats
  end
  ...
  def base_damage(**move, test)
    if SPECIAL_MOVE_TYPE.find { |category| category == move[:type] } != nil
      offensive_stat = @stats[:special_attack]
      test_defensive_stat = target.stats[:special_defense]
    else
  ...
  #Revisar: "every move has a 1/16 chance of being a critical hit"
  def critical_hit
    rand(1..16) >= rand(1..16)
  end
end
